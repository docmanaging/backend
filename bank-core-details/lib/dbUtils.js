...
async function registerBank(requestParams,requestBody) {
	...
	//	const readTable = "select * from CUSTOMER_USERS where ID='" + user_id + "' and user_type='wmuser' and role='admin'";
	//	console.log("query is "+readTable);
	//	const connQueryPromisified = util.promisify(connection.query).bind(connection);
	//	const results1 = await connQueryPromisified(readTable); 
	...
	//	else {			
			...
			//const readTable = "select * from CUSTOMER_USERS where USER_ID='" + user_name + "'";
			//console.log("query is "+readTable);
			//const connQueryPromisified = util.promisify(connection.query).bind(connection);
			//const results1 = await connQueryPromisified(readTable); 
		...
		//	if (results1.length < 1) 
		//	{
		//		var writeTable = "INSERT INTO CUSTOMER_USERS(USER_ID,CUST_ID,FIRST_NAME,MIDDLE_NAME,LAST_NAME,EMAIL_ID,ROLE,"
		//		writeTable += "PASSWORD,MOBILE_NUM,PRIMARY_CONTACT,ACTIVE,CREATED_BY,UPDATED_BY)"
		//		writeTable += "VALUES ('" + user_name + "','" + cust_id + "','" + first_name + "','" + middle_name + "','" + last_name + "',"
		//		writeTable += "'" + user_id + "','" + role + "','temp123456','" + mobile_num + "',FALSE,FALSE,'" + user_name + "','" + user_name + "')";
